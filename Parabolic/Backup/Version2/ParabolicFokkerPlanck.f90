...
dt=1000.
do i=1,np-1
 dt=min(dt,(0.45)*dx**2./C(px(i),0._8))
end do
...
do j=0,nx-1
 do i=0,np-1
  !Initial profile
  !print*,'i,p,dp, ', i,px(i),dp(i),((px(i)-1.)/dp(i))**2.
  !if( ((px(i)-1.)/dp(i))**2.>1.d-3 .and. ((px(i)-1.)/dp(i))**2.<100)then
  !   f(i,j)=((px(i)-1.)/dp(i))**2.
  !   f(i,j)=exp(-f(i,j))
  !else
     f(i,j)=px(i)**(-5.) 
  !end if
  !if(i<3.*np/4. .and. i>np/4.)then
  ! f(i,j)=1.+
  !else
  ! f(i,j)=1. 
  !end if
...
do i=0,np-1
  ...
  write(funit,*) px(i),log(fp(i))!+1.D-80
...
do i=1,nt
  ...
  do k=0,np-1
   ...
   if(mod(i,tSkip)==0)then
    write(funit,*) px(k),log(fp(k))!+1.d-80
   end if
  end do
