...
rhoR=1.
pR=1.
uR=-M1*sqrt(gam*pR/rhoR)
rhoL=rhoR*((gam+1.)*M1*M1)/(2.+(gam-1.)*M1*M1)
uL=uR*rhoR/rhoL
pL=(2.*gam*M1*M1-(gam-1.))/(gam+1.)

!M1=20.
!rhoL=1.
!pL=1.
!uL=M1*sqrt(gam*pL/rhoL)
!rhoR=rhoL*((gam+1.)*M1*M1)/(2.+(gam-1.)*M1*M1)
!uR=uL*rhoL/rhoR
!pR=(2.*gam*M1*M1-(gam-1.))/(gam+1.)
...
!Open boundaries on the left and right
do k=1,3
  u2(-1,k)=u2(0,k)
  u2(-2,k)=u2(0,k)
  u2(nx,k)=u2(nx-1,k)
