...
!TIME STEP PROBLEM: Convergence to wrong solution without subcycling. WHY? 
...
do j=0,nx-1
 do i=0,np-1
  !Pc1(j)=Pc1(j)+(4.*cl/3.)*(px(i)**3.)*f(i,j)*dp2(i)
...
do j=0,nx-1
  do i=0,np-1
    Pc1(j)=Pc1(j)+pi*(4.*cl/3.)*(px(i)**3.)*f(i,j)*dp2(i)
    Pc(j)=Pc(j)+pi*(4.*cl/3.)*g(i,j)
  end do
end do
Pc(-1)=Pc(0)
 ...
 do j=1,np-1
  ...
  do k=0,nx-1
    r(k)=g(j,k)+dt2*(Fp(j,k)-Fp(j-1,k)) &
        +dt2*(-(1./3.)*dudx(k)+q(j,k)*D(j)/(px(j)*px(j)))*g(j,k)!+dt2*Qi(j,k,2)  
  end do
  call tridiagSolveSpace(ftemp,r)
...
do k=1,np-1
 write(110,*) k,px(k),n(k,shockCell(1)-1),g(k,shockCell(1)-1),q(k,shockCell(1)-1)
end do

do j=0,nx-1
 write(111,*) j,xmin+dx*j,n(25,j),g(25,j),q(25,j)
end do
