...
!BOUNDARY CONDITION PROBLEM: Convergence to wrong solution without separate advection. With separate advection, 
...
!do j=0,nx-1
!   pres=(gam-1.)*(u1(j,3)-0.5*u1(j,2)*u1(j,2)/u1(j,1))
!  print*,'j,Pc,Pg,Pc1=',j,Pc(j),pres,Pc1(j)
!end do

do j=0,nx-1
 write(13,*) xmin+j*dx,f(5,j)!f(1,j)!fx(j)
end do
do j=0,np-1
...
!------------------------------------------ 
 ...
 do j=1,np-1
  ...
  do k=0,nx-1
   ...
   n(j,k)=ntemp(j,k)+dt2*(Fn(j,k)-Fn(j-1,k)) &
        -dt2*dudx(k)*n(j,k)+(dt2/(dx*dx))*(fnR-fnL)!&
        !+dt2*Qi(j,k,1)

   !Evolve first moment: g_i,j
   g(j,k)=gtemp(j,k)+dt2*(Fg(j,k)-Fg(j-1,k)) &
        +dt2*(-(4./3.)*dudx(k))*gtemp(j,k)+(dt2/(dx*dx))*(fgR-fgL)!&
...
!----------------------------------------------
do j=0,nx-1 
 !$omp parallel do shared(n,g,q,px,dt2,dx) &
 !$omp& private(dw,qtemp)  
 do k=1,np-1
  dw=px(k)/px(k-1)
  if(dw<0._8)then
...
do j=0,nx-1
  ...
  do k=0,np-1
    ...
    !Pc(j)=Pc(j)+(4.*cl/3.)*(px(i)**3.)*f(k,j)*dp2(k)
  end do
end do
Pc(-1)=Pc(0)
Pc(-2)=Pc(0)
