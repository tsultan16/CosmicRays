...
do i=1,np-1
 write(110,*) i,px(i),n(i,250),g(i,250),q(i,250)
end do
...
do k=1,np-1
 write(110,*) k,px(k),n(k,250),g(k,250),q(k,250)
end do
...
do jj=0,nx-1
  ...
  if(methodType==2)then
   ...
   Btx(jj)=-Btx(jj)+1.-dt3*Hx(jj,option)
...
function Hx(xk,option) result(hxx)
 
integer,intent(in)::xk,option
real*8::hxx
