...
do i=0,np-1
 do j=0,nx-1
  !Initial profile
  !f(i,j)=px(i)**(-5.)
  f(i,j)=100.*exp(-( (xmin+j*dx-0.8)/(1.*dx) )**2. )!
  !f(i,j)=100*exp(-( (pmin+i*dp(i)-5.)/(2.*dp(i)) )**2. ) 
 end do
 f(-1,j)=f(0,j)  
 f(np,j)=f(np-1,j)
end do
