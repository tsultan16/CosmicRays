...
do for [i=0:tSteps-1] {
  ...
  #set xlabel "x"
  set ylabel "Density"
  plot "output_fluid1.txt" every ::i*nx1+1::nx1+(i*nx1)-1 using 1:2 with lines lc rgb "red" title "dt2=1.5*dt" ,\
 "output_fluid2.txt" every ::i*nx2+1::nx2+(i*nx2)-1 using 1:2 with lines lc rgb "green" title "dt2=2*dt" ,\
 "output_fluid3.txt" every ::i*nx3+1::nx3+(i*nx3)-1 using 1:2 with lines lc rgb "blue" title "dt2=2.5*dt" ,\
 "output_fluid4.txt" every ::i*nx4+1::nx4+(i*nx4)-1 using 1:2 with lines lc rgb "purple" title "dt2=3.0*dt"
