...
!------------------------------------------------------------------------------------
integer,parameter::nt=5000!2000
...
integer,parameter::advectionOption=2 !1:combined with spatial diffusion operator, 2:separate advection operator

real*8,parameter::cl=100!speed of light
real*8::Pc(-2:nx+2),dudx(0:nx-1)

real*8::f(-1:np,-1:nx)
real*8::q(-1:np,-1:nx),n(-1:np,-1:nx),g(-1:np,-1:nx)
real*8::At(0:np-1),Bt(0:np-1),Ct(0:np-1)
real*8::Atx(0:nx-1),Btx(0:nx-1),Ctx(0:nx-1)
real*8::px(-2:np+3),dp(-1:np+2),dp2(-1:np+2),dw
